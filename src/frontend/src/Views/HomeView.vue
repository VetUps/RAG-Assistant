  ...
  <div class="main-content">
    ...
    <div class="search-container">
      <h1 class="search-title"
          @mouseenter="speak('Введите ваш запрос')"
          @mouseleave="stopSpeech">
        Введите ваш запрос
      </h1>
      ...
      <div v-if="searchResult && !currentChatHistory" class="chat-message ai-message">
        <div class="message-role"
            @mouseenter="speak('Ассистент')"
            @mouseleave="stopSpeech">Ассистент</div>
        <div class="message-content"
            @mouseenter="speak(searchResult.answer)"
            @mouseleave="stopSpeech">{{ searchResult.answer }}</div>
        <div class="message-meta">
          <div class="message-time">{{ formatMessageTime(new Date().toISOString()) }}</div>
          <div v-if="searchResult.sources && searchResult.sources.length > 0" class="message-sources"
              @mouseenter="speak('Источники: ' + searchResult.sources.join(', '))"
              @mouseleave="stopSpeech">
            <strong>Источники:</strong> {{ searchResult.sources.join(', ') }}
          </div>
          <div v-if="searchResult.processing_time" class="processing-time"
              @mouseenter="speak('Время ответа: ' + searchResult.processing_time + ' секунд')"
              @mouseleave="stopSpeech">
            <strong>Время ответа:</strong> {{ searchResult.processing_time }} сек
          </div>
        </div>
      </div>

      <div v-if="currentChatHistory">
        <div v-for="(message, index) in currentChatHistory.messages" :key="index"
          ...
          <div class="message-role"
          @mouseenter="speak(message.role)"
              @mouseleave="stopSpeech">{{ message.role === 'user' ? 'Вы' : 'Ассистент' }}</div>
          <div class="message-content"
              @mouseenter="speak(message.content)"
              @mouseleave="stopSpeech">{{ message.content }}</div>

          <div v-if="message.role === 'assistant'" class="message-meta">
            <div class="message-time">{{ formatMessageTime(message.timestamp) }}</div>
            <div v-if="message.sources && message.sources.length > 0" class="message-sources"
                @mouseenter="speak('Источники: ' + message.sources.join(', '))"
                @mouseleave="stopSpeech">
              <strong>Источники:</strong> {{ message.sources.join(', ') }}
            </div>
            <div v-if="message.processing_time" class="processing-time"
                @mouseenter="speak('Время ответа: ' + message.processing_time + ' секунд')"
                @mouseleave="stopSpeech">
              <strong>Время ответа:</strong> {{ message.processing_time }} сек
            </div>
          </div>

          <div v-else class="message-time">{{ formatMessageTime(message.timestamp) }}</div>
        </div>
      </div>
...
body.high-contrast-mode .processing-time {
  font-style: italic;
  color: #ffffff !important;
}

body.high-contrast-mode .message-meta {
  border-top: 1px solid #ffffff !important;
  color: #cccccc !important;
}

body.high-contrast-mode .message-sources {
  color: #ffffff !important;
}
...
body.high-contrast-mode .history-time {
  font-style: italic;
  color: white;
}
      ...
      try {
        ...
        if (this.currentChatHistory) {
          this.currentChatHistory.messages.push(
              ...
              {
                role: 'assistant',
                content: this.searchResult.answer,
                timestamp: new Date().toISOString(),
                sources: this.searchResult.sources || [],
          ...
          this.currentChatHistory = newHistoryItem;
        }

        this.searchQuery = '';
        this.speak("Поиск завершен");
    ...
    addToHistory(searchItem) {
      if (!searchItem.messages) {
        searchItem.messages = [
          ...
          {
            role: 'assistant',
            content: searchItem.answer,
            timestamp: searchItem.timestamp,
            sources: searchItem.sources || [],
            processing_time: searchItem.processing_time
          }
        ];
      }
            ...
            if (!item.messages) {
              ...
              item.messages.forEach(msg => {
                if (msg.role === 'assistant' && !msg.sources) {
                  ...
                  msg.processing_time = item.processing_time;
                }
              });
            }
          });
